    ...
    float bot_r, float bot_g, float bot_b, float bot_a, float cameraPitch)
{
    ...
    if (background_vao == 0)
    {
        ...
        const GLchar* fs_src =
            ...
            "void main()\n"
            "{\n"
            "frag_color = vec4(    (  (  1/abs(  int(  gln_perlin( vec3 ((gl_FragCoord.y + cp - time*20)/100.0, 10.43, (gl_FragCoord.y+ cp  - time*20)/100.0) )   ) - 0.2  )  )  *100.0  )  + (1/abs(int(gl_FragCoord.y+ cp  - time*20)%1000 - 1000.0))*100    , (1/abs(int(gl_FragCoord.y + cp - time*20)%1000 - 500.0))*100, (1/abs(int(gl_FragCoord.y+ cp  - time*20)%1000 - 700.0))*100,1.0);\n"
           // "frag_color = bot_color a* (1 - v_uv.y) + top_color * v_uv.y;\n"
            "}";
    ...
    GLuint time_loc = glGetUniformLocation(background_shader, "time");
